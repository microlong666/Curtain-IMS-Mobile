    ...
    <van-pull-refresh class="refresh" v-model="refreshing" @refresh="onRefresh">
      <div class="content">
        <van-list
          ...
          @load="onLoad"
        >
          <van-swipe-cell class="cell" v-for="item in adjuvantList" :key="item">
...
<script>
import { getAdjuvantList } from '@/api/items'
...
export default {
  ...
  methods: {
    ...
    async onLoad() {
      ...
      const res = await getAdjuvantList(this.params)
      if (res.data.code === 0) {
