  ...
  <div class="content" v-else>
    <van-form class="form">
      <van-cell-group inset>
        <van-cell title="主布名称" :value="items.name" />
        <van-cell title="主布型号" :value="items.model" />
        ...
        <van-cell title="对花位长(米)" :value="items.patternHeight" />
        <van-cell title="对花位宽(米)" :value="items.patternWidth" />
...
<script>
import { getMainFabricDetail } from '@/api/items'
...
export default {
  ...
  created() {
    this.getMainFabricDetail()
  },
  methods: {
    async getMainFabricDetail() {
      const res = await getMainFabricDetail({ id: this.id })
      if (res.data.success) {
