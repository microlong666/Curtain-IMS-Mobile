  ...
  <div class="content" v-else>
    <van-form class="form">
      <van-cell-group inset>
        <van-cell title="窗纱名称" :value="items.name" />
        <van-cell title="窗纱型号" :value="items.model" />
...
<script>
import { getSheerDetail } from '@/api/items'
...
export default {
  ...
  methods: {
    async getSheerDetail() {
      const res = await getSheerDetail({ id: this.id })
      if (res.data.success) {
